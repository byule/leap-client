"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Discovery = void 0;
const os_1 = __importDefault(require("os"));
const path_1 = __importDefault(require("path"));
const deep_equal_1 = __importDefault(require("deep-equal"));
const flat_cache_1 = __importDefault(require("flat-cache"));
const event_emitter_1 = require("@mkellsy/event-emitter");
const tinkerhub_mdns_1 = require("tinkerhub-mdns");
const hap_device_1 = require("@mkellsy/hap-device");
/**
 * Creates and searches the network for devices.
 * @private
 */
class Discovery extends event_emitter_1.EventEmitter {
    /**
     * Creates a mDNS discovery object used to search the network for devices.
     *
     * ```js
     * const discovery = new Discovery();
     *
     * discovery.on("Discovered", (device: ProcessorAddress) => {  });
     * discovery.search()
     * ```
     */
    constructor() {
        super();
        /*
         * Parses a service once discovered. If it fits the criteria, this will
         * emit a discovered event.
         */
        this.onAvailable = (service) => {
            if (!this.isProcessorService(service))
                return;
            const host = this.parseProcessorAddress(service);
            if (!this.isProcessorCached(host))
                this.emit("Discovered", host);
            this.cacheProcessor(host);
        };
        this.cache = flat_cache_1.default.load("discovery", path_1.default.join(os_1.default.homedir(), ".leap"));
        this.cached = this.cache.getKey("/hosts") || [];
        this.cache.setKey("/hosts", this.cached);
        this.cache.save(true);
    }
    /**
     * Starts searching the network for devices.
     */
    search() {
        this.stop();
        for (let i = 0; i < this.cached.length; i++) {
            this.emit("Discovered", this.cached[i]);
        }
        this.discovery = new tinkerhub_mdns_1.MDNSServiceDiscovery({ type: "lutron", protocol: tinkerhub_mdns_1.Protocol.TCP });
        this.discovery.onAvailable(this.onAvailable);
    }
    /**
     * Stops searching the network.
     */
    stop() {
        var _a;
        (_a = this.discovery) === null || _a === void 0 ? void 0 : _a.destroy();
    }
    /*
     * Determines if a processor host is currently cached.
     */
    isProcessorCached(host) {
        return this.cached.find((entry) => (0, deep_equal_1.default)(entry, host)) != null;
    }
    /*
     * Determines if a MDNS service is a processor.
     */
    isProcessorService(service) {
        const type = service.data.get("systype");
        if (type == null || typeof type === "boolean")
            return false;
        return true;
    }
    /*
     * Saves a processor host to disk cache.
     */
    cacheProcessor(host) {
        const index = this.cached.findIndex((entry) => entry.id === host.id);
        if (index >= 0) {
            this.cached[index] = host;
        }
        else {
            this.cached.push(host);
        }
        this.cache.setKey("/hosts", this.cached);
        this.cache.save();
    }
    /*
     * Transforms a MDNS service to a processor host.
     */
    parseProcessorAddress(service) {
        var _a;
        const target = this.discovery.serviceData.get(service.id).SRV._record.target;
        const addresses = [];
        for (let i = 0; i < ((_a = service.addresses) === null || _a === void 0 ? void 0 : _a.length); i++) {
            addresses.push({
                address: service.addresses[i].host,
                family: /^([\da-f]{1,4}:){7}[\da-f]{1,4}$/i.test(service.addresses[i].host)
                    ? hap_device_1.HostAddressFamily.IPv6
                    : hap_device_1.HostAddressFamily.IPv4,
            });
        }
        return {
            id: target.match(/[Ll]utron-(?<id>\w+)\.local/).groups.id.toUpperCase(),
            type: String(service.data.get("systype")),
            addresses,
        };
    }
}
exports.Discovery = Discovery;
